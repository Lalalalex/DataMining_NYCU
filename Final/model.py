...
class NewsRecommendationModel(nn.Module):
    def __init__(self, embedding_dim=768, hidden_dim=384, output_dim=768):
        ...
        self.fc = nn.Sequential(
            nn.Linear(hidden_dim * MAX_CLICKED_LEN, output_dim),
...
class DeepLearning(object):
    ...
    def validation(self, val_loader):
        self.model.eval()
        with torch.no_grad():
            with tqdm(val_loader, unit="batch", desc = 'Valid') as tepoch:
                total_loss = []
                total_auc = []
                for X_clicked_batch, X_impressions_batch, y_batch in tepoch:
...
def main():
    ...
    train_set, val_set = random_split(train_dataset, [cfg.split, 1 - cfg.split], generator1)
