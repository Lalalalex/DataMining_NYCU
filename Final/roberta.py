...
class NewsRecommendationDataset(Dataset):
    def __init__(self, base_root, history_seq_len=35, is_train=True):
        ...
        self.encoder_bert = RobertaModel.from_pretrained('roberta-base')
...
def main():
    ...
    print(f"Using {device} with {torch.cuda.device_count()} GPUs available.")

    # 構建DataLoader

    train_dataset = NewsRecommendationDataset(cfg.train_path, MAX_CLICKED_LEN, is_train=True)
    generator1 = torch.Generator().manual_seed(42)
    train_set, val_set = random_split(train_dataset, [0.95, 0.05], generator1)
    ...
    optimizer = Ranger21(lstm_model.parameters(), lr=cfg.learning_rate, num_epochs=cfg.num_epochs, num_batches_per_epoch=len(train_dataloader))
